...
void handle_files_request(int fd) {
  ...
  char *path = name_fix(request->path);

  long unsigned bSize = 0;
 
  if(strcmp(request->path,"/\r") == 0 || strcmp(request->path,"/")==0 ){
    status = 200;
    mime_type = "index.html";   
    data = buffer("./files/index.html", &bSize);
  }else if(stat(path, &fileStat) >=0 ) {
    if(S_ISREG(fileStat.st_mode) != 0){
      status = 200;
      mime_type = http_get_mime_type(path);
      data = buffer(path, &bSize);
  ...
  http_end_headers(fd);
    ...
    //if( strcmp(mime_type, "text/html") ||strcmp(mime_type,"text/css") || strcmp(mime_type,"text/plain"))
  //http_send_string(fd,data);
     //else http_send_data(fd,data,sizeof(data) );
  http_send_data(fd,data, bSize);
